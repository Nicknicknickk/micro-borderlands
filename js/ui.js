...
const CHAR_DATA = {
  zero:    { name:'ZER0',     color:'#00e5ff', icon:'⚔️',
             desc:'Assassin. Deadly at range and close quarters.',
             skills:['Headsh0t +10% Dmg/Lv','0ptics -10% CD/Lv','B0re +10% Crit/Lv'],
             special:'Phase Shift: Vanish & deal burst crit damage' },
  maya:    { name:'MAYA',     color:'#cc44ff', icon:'🔮',
             desc:'Siren. Elemental specialist, powerful shields.',
             skills:['Flicker +20% Elem/Lv','Ward +20% Shield/Lv','Mind\'s Eye +10% Dmg/Lv'],
             special:'Phase Shield: Deflects bullets, blasts nearby foes' },
  axton:   { name:'AXTON',    color:'#00ee44', icon:'💣',
             desc:'Commando. Deploys a turret to cover the battlefield.',
             skills:['Sentry +20% Turret/Lv','Ready +10% FR/Lv','Healthy +20% HP/Lv'],
             special:'Sabre Turret: Deploys an auto-firing turret' },
  salvador:{ name:'SALVADOR', color:'#ff3333', icon:'🔫',
             desc:'Gunzerker. Guns down anything in his path.',
             skills:['Inconceivable +10% FR/Lv','Hard to Kill +20% HP/Lv','Juggernaut -10% CD/Lv'],
             special:'Gunzerking: Double fire rate, take half damage' },
  krieg:   { name:'KRIEG',    color:'#ff8800', icon:'🪓',
             desc:'Psycho. Melee destroyer with insane survivability.',
             skills:['Bloodlust +20% Melee/Lv','Feed the Meat +20% HP/Lv','Embrace Pain -10% CD/Lv'],
             special:'Rampage: Massive melee damage + speed boost' },
  gaige:   { name:'GAIGE',    color:'#00ddff', icon:'🤖',
             desc:'Mechromancer. Summons Deathtrap robot companion.',
             skills:['Best Friends +20% Pet/Lv','My Shields +20% Shield/Lv','Typecast +10% Dmg/Lv'],
             special:'Deathtrap: Deploys electric robot combat pet' }
};
...
function renderCharSelect () {
  ...
  Object.entries(CHAR_DATA).forEach(([id, ch]) => {
    ...
    card.style.cssText = [
      ...
      'border:3px solid ' + (isSelected ? ch.color : '#2a1e0e'),
      'background:' + (isSelected ? 'rgba(0,0,0,0.85)' : 'rgba(255,255,255,0.03)'),
      'box-shadow:' + (isSelected ? '0 0 18px ' + ch.color + '66,inset 0 0 12px ' + ch.color + '22' : 'none'),
      'position:relative',
      'overflow:hidden'
    ].join(';');
    ...
    card.innerHTML += `
      <div style="display:flex;align-items:center;gap:10px;margin-bottom:10px;padding-left:8px;">
        ...
        ${isSelected ? '<div style="margin-left:auto;color:' + ch.color + ';font-size:0.75rem;font-weight:bold;letter-spacing:1px;">✓ SELECTED</div>' : ''}
      </div>
      ...
      <div style="padding-left:8px;margin-bottom:10px;">
        ${ch.skills.map(s => `<div style="color:#665533;font-size:0.68rem;margin-bottom:3px;">▸ ${s}</div>`).join('')}
      </div>
  ...
  if (info && selectedChar) {
    const currentSaved = localStorage.getItem('borderClass') || 'zero';
    info.textContent = selectedChar === currentSaved
      ? `Currently playing as ${CHAR_DATA[selectedChar].name} — select to continue`
      : `Switching to ${CHAR_DATA[selectedChar].name}`;
    info.style.color = selectedChar === currentSaved ? '#665533' : '#ffa500';
  }
}
