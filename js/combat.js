...
function loopLooter() {
  ...
  for(let i=lhEnemies.length-1;i>=0;i--){
    ...
    if(e.hp<=0){
      ...
      const isBoss=e.type==='boss_goliath'||e.type==='boss'||e.type==='raid_boss'||e.type==='crawmerax'||e.type==='pete'||e.type==='terramorphous'||e.type.includes('_boss');
      if(isBoss){
        playerCoins+=1000;lhExitPortal={x:WORLD_W/2,y:WORLD_H/2};gainExp(1500*mayhemMult);
        if(e.type==='raid_boss'||e.type==='crawmerax'||e.type==='pete'||e.type==='terramorphous'){for(let k=0;k<5;k++)genLoot(e.x+(Math.random()*160-80),e.y+(Math.random()*160-80),true);for(let k=0;k<5;k++)genLoot(e.x+(Math.random()*160-80),e.y+(Math.random()*160-80),false,false,false,false,true);}
