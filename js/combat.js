/* ==========================================
   COMBAT.JS — Enemy spawning, shooting, game loop
   ==========================================
   /* ==========================================
   COMBAT.JS — Full combat engine
   ========================================== */
...
function checkVehicleExplosion() {
  if (lhPlayer.vehicleHp <= 0) {
    inVehicle = false;
    playSound('explosion'); screenShake = 30;
    ...
    if (lhPlayer.shield > 0) {
      if (lhPlayer.shield >= recoilDmg) { lhPlayer.shield -= recoilDmg; recoilDmg = 0; }
      else { recoilDmg -= lhPlayer.shield; lhPlayer.shield = 0; }
    }
    lhPlayer.hp -= recoilDmg;
    lhPlayer.vehicleCooldown = 600;
...
function startLooter(raid = false, duelTarget = null, underdome = false) {
  ...
  if (isRaidBoss) {
    ...
    lhBossSpawned = true; playSound('die'); screenShake = 40;
  } else if (isDuel) {
    lhKills = 20;
    if (duelOpponent === 'Lilith') {
      lhEnemies.push({ x:WORLD_W/2, y:WORLD_H/2-200, hp:200000*mayhemMult, maxHp:200000*mayhemMult, speed:3, type:'lilith_boss', w:150, h:150, fT:0, sT:0, aT:0, cd:90 });
    } else {
      lhEnemies.push({ x:WORLD_W/2, y:WORLD_H/2-200, hp:200000*mayhemMult, maxHp:200000*mayhemMult, speed:2, type:'moxxi_boss', w:150, h:150, fT:0, sT:0, aT:0, cd:80 });
    }
    lhBossSpawned = true; playSound('die'); screenShake = 30;
  }
...
function loopLooter() {
  ...
  if (lhPlayer.dead || lhPlayer.win) {
    ...
    ctx.fillStyle = lhPlayer.win ? '#00ffcc' : '#ff3333';
    ...
    if (lhPlayer.win) {
      if (lhPlayer.winTimer === undefined) {
        lhPlayer.winTimer = 50;
        runCount++; badassTokens += 2;
        if (isRaidBoss) badassTokens += 10;
        localStorage.setItem('borderRuns', runCount);
        localStorage.setItem('badassTokens', badassTokens);
      }
      lhPlayer.winTimer--;
      if (lhPlayer.winTimer <= 0) { startSanctuary(); return; }
    } else {
      ctx.font = '25px Arial'; ctx.fillText('Click to Play Again', 400, 260);
      gCanvas.onclick = () => { gCanvas.onclick = null; startLooter(); };
    }
    ctx.textAlign = 'left';
    animId = requestAnimationFrame(loopLooter); return;
  }

  // ── Vehicle cooldown & repair ─────────────
  ...
  if (keys['KeyV'] || keys['v']) {
    ...
    if (hasVehicle && lhPlayer.vehicleCooldown <= 0 && !isRaidBoss && !isDuel) {
      inVehicle = !inVehicle; playSound('ability');
      spawnParticles(lhPlayer.x, lhPlayer.y, '#ff6600', 30, 4, 20);
  ...
  if (inVehicle && (keys['KeyW']||keys['ArrowUp']||keys['KeyS']||keys['ArrowDown']||keys['KeyA']||keys['ArrowLeft']||keys['KeyD']||keys['ArrowRight'])) {
  ...
  if ((keys['KeyE']||keys['e']) && lhPlayer.skillCooldown<=0 && !inVehicle) {
    ...
    if (!onPortal) {
      ...
      else if (lhPlayer.char==='maya') {
        lhPlayer.skillCooldown=400*cdMult; lhPlayer.skillTimer=180; spawnParticles(lhPlayer.x,lhPlayer.y,'#a020f0',50,8,40);
        lhDmgText.push({x:lhPlayer.x,y:lhPlayer.y,txt:'PHASE SHIELD!',life:60,c:'#a020f0'});
        lhEnemies.forEach(e => { if(Math.hypot(lhPlayer.x-e.x,lhPlayer.y-e.y)<500 && !e.type.includes('boss')) { e.hp-=500*mayhemMult; playSound('hit'); lhDmgText.push({x:e.x,y:e.y,txt:500*mayhemMult,life:30,c:'#a020f0'}); spawnParticles(e.x,e.y,'#a020f0',15,4,20); } });
      }
  ...
  if (lhPlayer.char==='krieg' && lhPlayer.skillTimer>0 && lhPlayer.skillTimer%10===0) {
    ...
    lhEnemies.forEach(e => {
      if (Math.hypot(lhPlayer.x-e.x,lhPlayer.y-e.y)<100) {
        e.hp -= meleeDmg; playSound('hit'); spawnParticles(e.x,e.y,'#ffaa00',10,4,15);
        lhDmgText.push({x:e.x,y:e.y,txt:Math.floor(meleeDmg),life:30,c:'#ffaa00'});
        if (e.hp<=0 && equippedCMod==='Meat') { lhPlayer.hp+=lhPlayer.maxHp*0.1; if(lhPlayer.hp>lhPlayer.maxHp) lhPlayer.hp=lhPlayer.maxHp; }
      }
    });
  }
  ...
  if ((keys['KeyG']||keys['g']) && lhPlayer.grenades>0 && lhPlayer.grenadeCooldown<=0 && !inVehicle) {
    ...
    lhPlayer.grenadeCooldown = 60; playSound('shoot');
    const gAng = Math.atan2(targetY-lhPlayer.y, targetX-lhPlayer.x);
    let gDmg = 400*(1+rankDmg*0.15);
    let gRad = 150;
    if (equippedGMod==='Fastball')      { gDmg*=3; gRad=50; }
    if (equippedGMod==='Bonus Package') { gRad=300; }
  ...
  if (lhShooting && lhPlayer.gun.timer<=0 && !inVehicle && lhPlayer.char!=='krieg') {
    if (!(lhPlayer.char==='krieg' && lhPlayer.skillTimer>0)) {
      ...
      if (gType==='Shotgun') {
        const pellets = isPearl ? 9 : 5;
        const count   = (lhPlayer.char==='salvador'&&lhPlayer.skillTimer>0) ? pellets*2 : pellets;
        for (let p=0; p<count; p++) fireBullet((Math.random()-0.5)*0.6, false);
      } else if (gType==='Launcher') {
        fireBullet(0, true);
        if (lhPlayer.char==='salvador'&&lhPlayer.skillTimer>0) fireBullet(0.2, true);
      } else {
        if (lhPlayer.char==='salvador'&&lhPlayer.skillTimer>0) {
          if (isPearl) { fireBullet(-0.2,false);fireBullet(0,false);fireBullet(0.2,false);fireBullet(-0.3,false);fireBullet(0.1,false);fireBullet(0.3,false); }
          else { fireBullet(-0.1,false); fireBullet(0.1,false); }
        } else {
          if (isPearl) { fireBullet(-0.15,false);fireBullet(0,false);fireBullet(0.15,false); }
          else fireBullet(0, false);
        }
      }
    }
  }

  // ── Ally updates (turret / deathtrap) ────
  for (let i=lhAllies.length-1; i>=0; i--) {
    ...
    if (a.type==='deathtrap') {
      ...
      if (a.cd<=0 && nearest) {
        playSound('shoot');
        const tDmg = 80*mayhemMult*(1+rankDmg*0.15)*(1+lhPlayer.mods.pet);
        const tAng = Math.atan2(nearest.y-a.y,nearest.x-a.x);
        lhBullets.push({x:a.x,y:a.y,vx:Math.cos(tAng)*15,vy:Math.sin(tAng)*15,dmg:tDmg,c:'#0ff',hitList:[]}); a.cd=10;
      }
    } else {
      if (a.cd<=0 && nearest) {
        playSound('shoot');
        const tDmg = 50*mayhemMult*(1+rankDmg*0.15)*(1+lhPlayer.mods.pet);
        const tAng = Math.atan2(nearest.y-a.y,nearest.x-a.x);
        lhBullets.push({x:a.x,y:a.y,vx:Math.cos(tAng)*12,vy:Math.sin(tAng)*12,dmg:tDmg,c:'#0f0',hitList:[]}); a.cd=15;
      }
    }
  ...
  if (isUnderdome) {
    ...
    if (!isRaidBoss&&!isDuel&&!lhBossSpawned&&lhKills>=20) {
      ...
      playSound('die'); spawnParticles(bossX,bossY,'#800080',60,6,50);
    }
  ...
  if (lhExitPortal) {
    ...
    if (Math.hypot(lhPlayer.x-lhExitPortal.x,lhPlayer.y-lhExitPortal.y)<50 && (keys['KeyE']||keys['e']) && !inVehicle) {
      keys['KeyE']=false; keys['e']=false; lhPlayer.win=true;
    }
  }
  ...
  for (let i=lhGrenades.length-1;i>=0;i--) {
    ...
    if (g.timer<=0) {
      playSound('explosion'); spawnParticles(g.x,g.y,'#ff8c00',60,8,40); screenShake=20;
      lhEnemies.forEach(e => {
        if (Math.hypot(g.x-e.x,g.y-e.y)<g.radius) {
          e.hp-=g.dmg; lhDmgText.push({x:e.x,y:e.y,txt:Math.floor(g.dmg),life:40,c:'#ff0'});
  ...
  for (let i=lhEnemies.length-1;i>=0;i--) {
    ...
    // Loader death timer
    if (e.type==='loader' && e.hp<=0 && e.deathTimer===undefined) { e.deathTimer=60; e.speed=0; playSound('hit'); continue; }
    if (e.deathTimer!==undefined) {
      e.deathTimer--;
      if (e.deathTimer<=0) {
        playSound('explosion'); spawnParticles(e.x,e.y,'#ff4500',100,8,50); screenShake=30;
        const dist=Math.hypot(lhPlayer.x-e.x,lhPlayer.y-e.y);
        if (dist<120) {
          if (inVehicle) { lhPlayer.vehicleHp-=1000; checkVehicleExplosion(); } else {
            let dmg=200*mayhemMult*(1-equippedArmor.dmgRed); lhPlayer.shieldRechargeDelay=240;
            if(lhPlayer.shield>0){if(lhPlayer.shield>=dmg){lhPlayer.shield-=dmg;dmg=0;}else{dmg-=lhPlayer.shield;lhPlayer.shield=0;}}
            lhPlayer.hp-=dmg; spawnParticles(lhPlayer.x,lhPlayer.y,'#ff0000',20,5,20);
            if(lhPlayer.hp<=0) lhPlayer.dead=true;
          }
        }
        ...
        if(Math.random()<0.3) genLoot(e.x,e.y,false);
        lhEnemies.splice(i,1);
      }
      continue;
    }
    ...
    if (e.cd!==undefined) { e.cd--;
      if (e.cd<=0) {
        ...
        if (e.type==='crawmerax') {
          playSound('die'); screenShake=15;
          lhEnemyBullets.push({x:lhPlayer.x,y:lhPlayer.y-200,vx:0,vy:10,dmg:100*mayhemMult,c:'#0ff',isSpike:true}); e.cd=40;
        } else if (e.type==='pete') {
          playSound('explosion'); screenShake=20;
          for(let r=0;r<30;r++){const a=(Math.PI*2/30)*r;lhEnemyBullets.push({x:e.x,y:e.y,vx:Math.cos(a)*8,vy:Math.sin(a)*8,dmg:30*mayhemMult,c:'#ff4500'});} e.cd=120;
        } else if (e.type==='terramorphous') {
          playSound('die');
          for(let r=0;r<16;r++){const a=(Math.PI*2/16)*r;lhEnemyBullets.push({x:e.x,y:e.y,vx:Math.cos(a)*7,vy:Math.sin(a)*7,dmg:50*mayhemMult,c:'#0f0'});} e.cd=60;
        } else if (e.type==='lilith_boss') {
          playSound('shoot');
          for(let w=1;w<=5;w++){lhEnemyBullets.push({x:e.x,y:e.y,vx:Math.cos(bAng-0.2*w)*6,vy:Math.sin(bAng-0.2*w)*6,dmg:1.5*mayhemMult,c:'#ff4500'});lhEnemyBullets.push({x:e.x,y:e.y,vx:Math.cos(bAng+0.2*w)*6,vy:Math.sin(bAng+0.2*w)*6,dmg:1.5*mayhemMult,c:'#ff4500'});} e.cd=60;
        } else if (e.type==='moxxi_boss') {
          playSound('hit');
          for(let w=-2;w<=2;w++) lhEnemyBullets.push({x:e.x,y:e.y,vx:Math.cos(bAng+w*0.15)*6,vy:Math.sin(bAng+w*0.15)*6,dmg:2*mayhemMult,c:'#ff007f'});
          const tOff=Date.now()/250;
          for(let r=0;r<12;r++){const a=(Math.PI*2/12)*r;lhEnemyBullets.push({x:e.x,y:e.y,vx:Math.cos(a+tOff)*3,vy:Math.sin(a+tOff)*3,dmg:2*mayhemMult,c:'#ff00ff'});} e.cd=50;
        } else if (runCount>=3) {
          ...
          let ang;
          if ((runCount>=5||e.type==='loader')&&e.isMarksman) {
            const pvx=lhPlayer.x-lhPlayer.lastX, pvy=lhPlayer.y-lhPlayer.lastY;
    ...
    // Elemental DoTs
    if(e.fT>0){e.fT--;if(e.fT%30===0){const d=Math.floor(10*fireLvl*elemMult);e.hp-=d;lhDmgText.push({x:e.x+(Math.random()*20-10),y:e.y-20,txt:d,life:30,c:'#ff4500'});playSound('hit');}if(e.fT%5===0)lhParticles.push({x:e.x+(Math.random()*20-10),y:e.y+10,vx:(Math.random()-0.5),vy:-2-Math.random()*2,c:'#ff4500',life:20,maxLife:20,size:Math.random()*4+2});}
    if(e.sT>0){e.sT--;if(e.sT%30===10){const d=Math.floor(10*shockLvl*elemMult);e.hp-=d;lhDmgText.push({x:e.x+(Math.random()*20-10),y:e.y-20,txt:d,life:30,c:'#00ffff'});playSound('hit');}if(e.sT%8===0)lhParticles.push({x:e.x,y:e.y,vx:(Math.random()-0.5)*8,vy:(Math.random()-0.5)*8,c:'#00ffff',life:10,maxLife:10,size:2});}
    if(e.aT>0){e.aT--;if(e.aT%30===20){const d=Math.floor(10*acidLvl*elemMult);e.hp-=d;lhDmgText.push({x:e.x+(Math.random()*20-10),y:e.y-20,txt:d,life:30,c:'#32cd32'});playSound('hit');}if(e.aT%10===0)lhParticles.push({x:e.x+(Math.random()*20-10),y:e.y+(Math.random()*20-10),vx:0,vy:-0.5,c:'#32cd32',life:40,maxLife:40,size:Math.random()*6+3});}
    ...
    if (e.hp<=0) {
      ...
      if (isBoss) {
        ...
        lhKills++;playerCoins+=20;gainExp(50*mayhemMult);
        if(e.pref==='Loot'){genLoot(e.x,e.y,false);genLoot(e.x+10,e.y+10,false);genLoot(e.x-10,e.y-10,false);}
        else if(Math.random()<0.20) genLoot(e.x,e.y,false);
        if(activeQuest===1&&e.type==='normal'){questProgress++;localStorage.setItem('borderQProg',questProgress);}
      }
      localStorage.setItem('borderCoins',playerCoins);
      lhEnemies.splice(i,1); continue;
    }
    ...
    // Draw sprite
    if(e.type==='boss_goliath'){ctx.fillStyle='white';ctx.font='16px Courier';ctx.fillText('BADASS GOLIATH',e.x-e.w/2,e.y-e.h/2-20);if(goliathImg.complete&&goliathImg.naturalHeight!==0)ctx.drawImage(goliathImg,e.x-e.w/2,e.y-e.h/2,e.w,e.h);else drawPixelSprite(ctx,'goliath',e.x,e.y,e.w);}
    ...
    // Contact damage
    const dist=Math.hypot(lhPlayer.x-e.x,lhPlayer.y-e.y);
    if(inVehicle&&dist<(e.w/2+25)){playSound('hit');e.hp-=100;spawnParticles(e.x,e.y,'#ffcc00',20,6,20);lhDmgText.push({x:e.x,y:e.y-20,txt:'RAM!',life:30,c:'#ffcc00'});const ra=Math.atan2(e.y-lhPlayer.y,e.x-lhPlayer.x);e.x+=Math.cos(ra)*60;e.y+=Math.sin(ra)*60;lhPlayer.vehicleHp-=(e.type.includes('boss')?500:50);checkVehicleExplosion();}
    else if(!inVehicle&&dist<(e.w/2+10)&&lhPlayer.skillTimer<=0){
      playSound('die');
      let dmgTaken=e.type==='boss_goliath'?25:e.type==='goliath'?10:e.type==='loader'?5:e.type.includes('boss')||e.type==='crawmerax'||e.type==='pete'?100:3;
      ...
      spawnParticles(lhPlayer.x,lhPlayer.y,lhPlayer.shield>0?'#00aaff':'#ff0000',10,4,15);
  ...
  for(let i=lhBullets.length-1;i>=0;i--){
    ...
    for(let j=lhEnemies.length-1;j>=0;j--){
      ...
      if(Math.hypot(b.x-e.x,b.y-e.y)<(e.w/2+(b.isRocket?10:4))){
        if(b.isRocket){
          playSound('explosion');spawnParticles(b.x,b.y,'#ff8c00',40,6,30);screenShake=10;
          ...
          playSound('hit');
          const cChance=0.15+lhPlayer.mods.crit+(b.isSniper?0.30:0);
          const cMult=b.isSniper?5:2.5;
          ...
          if(fireLvl>0)e.fT=elemDur;if(shockLvl>0)e.sT=elemDur;if(acidLvl>0)e.aT=elemDur;
          b.hitList.push(e);hitSomething=true;
          if(!b.pierce){lhBullets.splice(i,1);break;}
        }
      }
    }
  ...
  for(let i=lhEnemyBullets.length-1;i>=0;i--){
    const b=lhEnemyBullets[i];b.x+=b.vx;b.y+=b.vy;
    if(b.isSpike){ctx.fillStyle=b.c;ctx.fillRect(b.x-2,b.y-10,4,20);}
    ...
    if(dist<15&&!inVehicle){
      playSound('die');let dmgTaken=b.dmg*(1-equippedArmor.dmgRed);
      if(lhPlayer.char==='salvador'&&lhPlayer.skillTimer>0)dmgTaken*=0.5;
      if(lhPlayer.char==='krieg'&&lhPlayer.skillTimer>0)dmgTaken*=0.5;
      lhPlayer.shieldRechargeDelay=180;
      if(lhPlayer.shield>0){if(lhPlayer.shield>=dmgTaken){lhPlayer.shield-=dmgTaken;dmgTaken=0;}else{dmgTaken-=lhPlayer.shield;lhPlayer.shield=0;}}
      lhPlayer.hp-=dmgTaken;spawnParticles(lhPlayer.x,lhPlayer.y,lhPlayer.shield>0?'#00aaff':'#ff0000',10,4,15);
      if(lhPlayer.hp<=0)lhPlayer.dead=true;lhEnemyBullets.splice(i,1);
    }else if(inVehicle&&dist<35){playSound('hit');lhPlayer.vehicleHp-=b.dmg;spawnParticles(b.x,b.y,'#ff6600',5,2,10);lhEnemyBullets.splice(i,1);checkVehicleExplosion();}
  }
  ...
  if(inVehicle){
    ...
    ctx.fillStyle='#ff6600';ctx.fillRect(10,10,(Math.max(0,lhPlayer.vehicleHp)/lhPlayer.maxVehicleHp)*150,15);
    ctx.strokeStyle='#fff';ctx.strokeRect(10,10,150,15);
    ctx.fillStyle='#fff';ctx.font='12px Arial';ctx.fillText(`ARMOR: ${Math.floor(lhPlayer.vehicleHp)}/${lhPlayer.maxVehicleHp}`,15,22);
    ctx.fillText(`Credits: $${playerCoins}`,10,45);
  } else {
  ...
  if(isUnderdome){
    ...
    ctx.fillStyle='#fff';ctx.font='14px Courier';ctx.fillText(`Enemies Remaining: ${underdomeEnemies+lhEnemies.length}`,400,102);ctx.textAlign='left';
  }
