...
function checkVehicleExplosion() {
  if (lhPlayer.vehicleHp <= 0) {
    inVehicle = false;
    playSound('explosion', lhPlayer.x); screenShake = 30;
...
function loopLooter() {
  ...
  if (lhPlayer.dead || lhPlayer.win) {
    ...
    if (lhPlayer.win) {
      if (lhPlayer.winTimer===undefined) {
        lhPlayer.winTimer=50; runCount++; badassTokens+=2;
        if (isRaidBoss) badassTokens+=10;
        localStorage.setItem('borderRuns',runCount); localStorage.setItem('badassTokens',badassTokens);
      }
  ...
  if ((keys['KeyE']||keys['e'])&&lhPlayer.skillCooldown<=0&&!inVehicle) {
    ...
    if (!onPortal) {
      ...
      if (lhPlayer.char==='zero')     { lhPlayer.skillTimer=180; lhPlayer.skillCooldown=600*cdMult; spawnParticles(lhPlayer.x,lhPlayer.y,'#0ff',40,6,30); }
        ...
        lhEnemies.forEach(e=>{if(Math.hypot(lhPlayer.x-e.x,lhPlayer.y-e.y)<500&&!e.type.includes('boss')){e.hp-=500*mayhemMult;playSound('hit',e.x);lhDmgText.push({x:e.x,y:e.y,txt:500*mayhemMult,life:30,c:'#a020f0'});spawnParticles(e.x,e.y,'#a020f0',15,4,20);}});
      }
  ...
  if (lhPlayer.char==='krieg'&&lhPlayer.skillTimer>0&&lhPlayer.skillTimer%10===0) {
    ...
    lhEnemies.forEach(e=>{
      if (Math.hypot(lhPlayer.x-e.x,lhPlayer.y-e.y)<100) {
        e.hp-=meleeDmg; playSound('hit',e.x); spawnParticles(e.x,e.y,'#ffaa00',10,4,15);
  ...
  if (lhShooting&&lhPlayer.gun.timer<=0&&!inVehicle&&lhPlayer.char!=='krieg') {
    if (!(lhPlayer.char==='krieg'&&lhPlayer.skillTimer>0)) {
      ...
      if(gType==='Shotgun'){
        ...
        fireBullet(0,true); if(lhPlayer.char==='salvador'&&lhPlayer.skillTimer>0) fireBullet(0.2,true);
      } else {
  ...
  for(let i=lhEnemies.length-1;i>=0;i--){
    ...
    // Contact damage
    const dist=Math.hypot(lhPlayer.x-e.x,lhPlayer.y-e.y);
    if(inVehicle&&dist<(e.w/2+25)){
      playSound('hit',e.x);e.hp-=100;spawnParticles(e.x,e.y,'#ffcc00',20,6,20);
      lhDmgText.push({x:e.x,y:e.y-20,txt:'RAM!',life:30,c:'#ffcc00'});
      const ra=Math.atan2(e.y-lhPlayer.y,e.x-lhPlayer.x);e.x+=Math.cos(ra)*60;e.y+=Math.sin(ra)*60;
      lhPlayer.vehicleHp-=(e.type.includes('boss')?500:50);checkVehicleExplosion();
